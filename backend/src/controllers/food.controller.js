...
const seedFoods = async () => {
  try {
    ...
    await FoodItem.insertMany(fallbackFoods);
    console.log('Food items seeded!');
  } catch (err) {
    console.warn('Food seeding skipped, using fallback items:', err.message);
  }
};

const getFallbackItems = (filter = {}) => {
  let items = fallbackFoods.filter((item) => item.isAvailable !== false);
...
exports.getAllFoods = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('Food DB lookup failed, using fallback items:', dbErr.message);
    }

    if (!foods.length) {
      foods = getFallbackItems(filter);
    }
...
exports.getFeatured = async (req, res) => {
  try {
    ...
    let foods = [];
    try {
      foods = await FoodItem.find({ isFeatured: true, isAvailable: true });
    } catch (dbErr) {
      console.warn('Featured food DB lookup failed, using fallback items:', dbErr.message);
    }

    if (!foods.length) {
      foods = getFallbackItems({ isFeatured: true });
    }
